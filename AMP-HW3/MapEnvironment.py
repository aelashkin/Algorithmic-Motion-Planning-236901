...
class MapEnvironment(object):
    ...
    def compute_union_of_points(self, points1, points2):
        ...
        '''
        # print("points1 is {}".format(points1))
        # print("points2 is {}".format(points2))
        if points1.size == 0:
            return points2
        if points2.size == 0:
            return points1

        union = np.concatenate((points1, points2), axis=0)
        res = np.unique(union, axis=0)
        # print("points in res are {}".format(res))
        return res

        # return list(set.union(points1, points2))
