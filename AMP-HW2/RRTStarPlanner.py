...
class RRTStarPlanner(object):
    ...
    def __init__(self, planning_env, ext_mode, goal_prob, k, true=None):
    ...
    def plan(self):
        ...
        plan.reverse()
        plan = np.array(plan)
        print("next id is:", next_id, type(next_id))
        print("The plan is: ", plan, type(plan))
        # print total path cost and time
        #print("tree vertex is:", self.tree.vertices)
        #for vertex in self.tree.vertices:
            #print("vertex is:", vertex.state, "cost is:", vertex.cost)
    ...
    def compute_cost(self, plan):
        ...
        for i in range(len(plan) - 1):
            ...
            print("Edge price is:", edge_price)
    ...
    def expand_tree(self):
        ...
        while True:
            ...
            if self.planning_env.state_validity_checker(new_node) and self.planning_env.edge_validity_checker(
                ...
                print("new_node is:", new_node, type(new_node))
                knn_ids, knn_list = self.tree.get_k_nearest_neighbors(new_node, self.k, self.k_log)
                for near_id in knn_ids:
                    self.rewire_rrt(near_id, new_node_id)
                for near_id in knn_ids:
                    self.rewire_rrt(new_node_id, near_id)
    ...
    def rewire_rrt(self, potential_parent_id, child_id):
        child = self.tree.vertices[child_id]
        potential_parent = self.tree.vertices[potential_parent_id]

        if self.planning_env.edge_validity_checker(potential_parent.state, child.state):
            cost = self.planning_env.compute_distance(potential_parent.state, child.state)
            previous_cost = potential_parent.cost
            potential_cost = previous_cost + cost
            #potential_cost = self.tree.vertices(potential_parent_id).cost + cost
            if potential_cost < child.cost:
                self.tree.add_edge(potential_parent_id, child_id, potential_cost)
                #self.vertices[child_id].set_cost(cost=self.vertices[sid].cost + edge_cost)
                #self.tree.SetVertexCost(child_id, potential_cost)
